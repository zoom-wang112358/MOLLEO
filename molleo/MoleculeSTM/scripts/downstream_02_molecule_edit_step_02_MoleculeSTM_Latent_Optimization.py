...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_SMILES_file", type=str, default="/scratch/ssd004/scratch/mskrt/datasets/Editing_data/single_multi_property_SMILES.txt")
    ...
    for description in description_list:
        ...
        #source_SMILES_list = ["CCCC=C(C=CCOP(CCCCCCCCCCCCCCCC(=O)O)OCCCCCCCCCCCCCCCC)CC=CC=C(C)c1ccc(OC=O)cc1"]
        #source_SMILES_list=['CCCCCCCCCCCCCCCCOP(=O)(O)Oc1ccc(C=Cc2ccc(OP(=O)(O)OCCCCCCCCCCCCCCCC)cc2)cc1']
        for SMILES in source_SMILES_list[:5]:
            print("===== for SMILES {} =====".format(SMILES))
            result_SMILES_list_, result_acc_list_ = check_edit(SMILES, description)
            # result_SMILES_list_: [text_prompt, input_smiles, recontructed_smiles, generated_smiles, lambda]
